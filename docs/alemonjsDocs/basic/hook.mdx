...
export default async event => {
  const [reaction] = useReaction(event)
...
export default async event => {
  const [permission] = usePermission(event)
...
export default async event => {
  const [announce] = useAnnounce(event)
