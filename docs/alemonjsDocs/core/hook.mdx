...
> 如果要实现带状态的多轮交互流程，推荐使用[事件上下文](/docs/alemonjsDocs/core/context)。`useSubscribe` 更适合临时事件监听和生命周期订阅。
```
...
`upload`、`sendChannel` 和 `sendUser` 支持 `url`、`data`、`filePath`、`fileId` 四种媒体来源，但一次只能提供一种来源。针对具有明确目标的群组或 C2C 场景，也可以使用：

```ts
await media.send({
  target: { targetId: '123', scope: 'channel' },
  type: 'image',
...
```ts
import { useConnection } from 'alemonjs'

const [{ getStatus }] = useConnection()
const result = await getStatus({ BotId: 'bot-id' })
```

如果平台不支持该能力，会返回 `Warn` 结果；调用方应通过返回值的 `code` 判断结果。
...
如果不传 `InteractionId`，Hook 会尝试从当前事件上下文读取；如果平台不支持交互确认，会返回 `Warn` 结果。
...
## `Forum&Audio&ChannelSettings`

| Hook                 | 能力                                           |
| -------------------- | ---------------------------------------------- |
| `useSchedule`        | 查询、创建、更新和删除定时内容                 |
| `useForum`           | 查询、创建和删除帖子 / 主题                    |
| `useAudio`           | 控制音频、加入或离开音频频道、查询在线状态     |
| `useChannelSettings` | 查询消息频率、权限，申请接口权限和打开私聊会话 |

```ts
import { useSchedule, useForum } from 'alemonjs'

const [{ list }] = useSchedule()
const schedules = await list({ channelId: 'channel-id' })
...
`useMenu` 管理会话菜单；`usePanel` 管理命令面板及其可见目标。命令面板支持命令、链接、管理员限制和目标用户 / 群组：

```ts
import { useMenu, usePanel } from 'alemonjs'

const [{ set }] = useMenu()
await set({
  items: [
    { name: '帮助', type: 'message', text: '/help' },
    { name: '文档', type: 'link', url: 'https://alemonjs.com' }
  ]
})

const [{ list }] = usePanel()
const panels = await list({ scope: 'group' })
```
