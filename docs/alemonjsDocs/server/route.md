...
- **路径遍历防护**：所有 `/app/*` 和 `/apps/:app/*` 路由的文件路径经过 `safePath()` 校验，`../` 等遍历尝试返回 **403**
- **包名校验**：`/apps/:app/*` 路由校验应用名是否为合法 npm 包名，非法名称返回 **400**

:::warning

接口路由基于文件系统映射，请确保 `src/route/` 目录下不存放敏感文件。接口认证/授权请通过[中间件](./route-mw.md)实现。
